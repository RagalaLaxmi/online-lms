...
if (!$course) {
    echo "Course not found or you don't have permission.";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("UPDATE courses SET course_name = ?, course_description = ?, video_path = ? WHERE id = ? AND user_id = ?");
        $stmt->execute([$course_name, $course_description, $video_url, $course_id, $_SESSION['user_id']]);
        $success = true;
...
<body>
    ...
    <form method="POST" action="">
        ...
        <input type="text" id="course_name" name="course_name" required value="<?= htmlspecialchars($course['course_name']) ?>">

        <label for="course_description">Course Description</label>
        <textarea id="course_description" name="course_description" required><?= htmlspecialchars($course['course_description']) ?></textarea>

        <label for="video_url">YouTube Video URL (optional)</label>
        <input type="text" id="video_url" name="video_url" placeholder="https://www.youtube.com/watch?v=VIDEO_ID" value="<?= htmlspecialchars($course['video_path']) ?>">
